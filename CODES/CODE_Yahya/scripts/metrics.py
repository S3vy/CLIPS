...
#************BJI************
"""
Same as above, except for ponderation of surfels in accordance with there distance 
to the other mask, it's not black or white it's a continuous linear function.
(np.ones(surfel_areas_gt[distances_gt_to_pred <= tolerance_mm].shape)-distances_gt_to_pred[distances_gt_to_pred <= tolerance_mm]/tolerance_mm))
 or more simply (1-d(gt,pred)/tolerance) is this factor
"""
def compute_border_jaccard_index(surface_distances, tolerance_mm):

  distances_gt_to_pred = surface_distances["distances_gt_to_pred"]
  distances_pred_to_gt = surface_distances["distances_pred_to_gt"]
  surfel_areas_gt = surface_distances["surfel_areas_gt"]
  surfel_areas_pred = surface_distances["surfel_areas_pred"]
  overlap_gt = np.sum(surfel_areas_gt[distances_gt_to_pred <= tolerance_mm]*(np.ones(surfel_areas_gt[distances_gt_to_pred <= tolerance_mm].shape)-(distances_gt_to_pred[distances_gt_to_pred <= tolerance_mm]/tolerance_mm)**2))
  overlap_pred = np.sum(surfel_areas_pred[distances_pred_to_gt <= tolerance_mm]*(np.ones(surfel_areas_pred[distances_pred_to_gt <= tolerance_mm].shape)-(distances_pred_to_gt[distances_pred_to_gt <= tolerance_mm]/tolerance_mm)**2))
  bji = (overlap_gt + overlap_pred) / (
      np.sum(surfel_areas_gt) + np.sum(surfel_areas_pred))
  return bji
...
def load_fdata(gt,tmtv_pred): #gt et tmtv_pred sont des fichiers .nii

    mask_gt=nib.load(gt).get_fdata()
    affine=nib.load(gt).affine
    mask_pred=nib.load(tmtv_pred).get_fdata()
    spacing_mm=np.array([np.linalg.norm(affine[:,0][:3]),np.linalg.norm(affine[:,1][:3]),np.linalg.norm(affine[:,2][:3])]) #normes de chaque vecteur colonne de la partie linéaire 3x3 de la matrice affine(4x4)
    
    return mask_gt,mask_pred,spacing_mm

#returns Normalized Surface Distance with a certain tolerance 
def nsd(mask_gt,mask_pred,spacing_mm, tolerance_mm):
    surface_distances= compute_surface_distances(mask_gt.astype(bool),mask_pred.astype(bool),np.abs(spacing_mm))
    return compute_surface_dice_at_tolerance(surface_distances,tolerance_mm)

#returns Border Jaccard Index
def bji(mask_gt,mask_pred,spacing_mm, tolerance_mm):
   surface_distances= compute_surface_distances(mask_gt.astype(bool),mask_pred.astype(bool),np.abs(spacing_mm))
   return compute_border_jaccard_index(surface_distances, tolerance_mm)
...
"""Computes the robust Hausdorff distance.
  ...
  Returns:
    a float value. The robust Hausdorff distance in mm.
  """
def hausdorff(mask_gt,mask_pred,spacing_mm, percentile=100):
    surface_distances= compute_surface_distances(mask_gt.astype(bool),mask_pred.astype(bool),np.abs(spacing_mm))
    return compute_robust_hausdorff(surface_distances,percentile)
...
AVERAGE DISTANCE    
    ...
    A tuple with two float values:
      ...
      - the average distance from the predicted surface to the ground truth
        surface.
"""
def average_distance(mask_gt,mask_pred,spacing_mm):
    surface_distances= compute_surface_distances(mask_gt.astype(bool),mask_pred.astype(bool),np.abs(spacing_mm))
    return compute_average_surface_distance(surface_distances)

def dsc(mask_gt,mask_pred):
    return compute_dice_coefficient(mask_gt.astype(bool), mask_pred.astype(bool))
...
def matriceToTMTV(matrice, affine):
    roi_mask = (matrice!= 0)

    voxel_dimensions = np.linalg.det(affine[:3,:3]) #Calcul du determinant de la matrice 3x3 (partie linéaire), ce qui correspond au volume du voxel
    voxel_volume = np.abs(np.prod(voxel_dimensions))
    num_voxels_in_roi = np.sum(roi_mask)
    roi_volume = num_voxels_in_roi * voxel_volume*0.001

    return round(roi_volume,2)

def faux_negatifs(file_seg, file_gt):
    matrice_seg=np.array(fileToMatrix(file_seg)[0])
    matrice_gt=np.array(fileToMatrix(file_gt)[0])
    mask_positif= (matrice_seg==0)*(matrice_gt!=0)
    return matriceToTMTV(mask_positif,fileToMatrix(file_seg)[1])
   

def faux_positifs(file_seg, file_gt):
    matrice_seg=np.array(fileToMatrix(file_seg)[0])
    matrice_gt=np.array(fileToMatrix(file_gt)[0])
    mask_positif= (matrice_seg!=0)*(matrice_gt==0)
    return matriceToTMTV(mask_positif,fileToMatrix(file_seg)[1])

def faux_positifs_bis(file_seg, file_gt):
    matrice_seg=np.array(fileToMatrix(file_seg)[0])
    matrice_gt=np.array(fileToMatrix(file_gt)[0])
    tensor_gt=torch.from_numpy(matrice_gt)
    tensor_gt=tensor_gt.to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
    tensor_gt=tensor_gt.squeeze()
    if len(tensor_gt.shape)>3:
        tensor_sum = tensor_gt.sum(dim=-1)
    else:
        tensor_sum = tensor_gt
    matrice_gt=tensor_sum.cpu().numpy()
    mask_positif= (matrice_seg!=0)*(matrice_gt==0)
    return matriceToTMTV(mask_positif,fileToMatrix(file_seg)[1])

def vrai_positifs(file_seg,file_gt):
    matrice_seg=np.array(fileToMatrix(file_seg)[0])
    matrice_gt=np.array(fileToMatrix(file_gt)[0])
    mask_positif= (matrice_seg!=0)*(matrice_gt!=0)
    return matriceToTMTV(mask_positif,fileToMatrix(file_seg)[1])

def vrai_negatifs(file_seg,file_gt):
    matrice_seg=fileToMatrix(file_seg)
    matrice_gt=fileToMatrix(file_gt)
    mask_positif= (matrice_seg[0]==0 and matrice_gt[0]==0)
    return matriceToTMTV(mask_positif,matrice_seg[1])

def tmtv(file):
    a=fileToMatrix(file)
    return matriceToTMTV(a[0],a[1])
